    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        ...
        @foreach($cards as $card)
            <a href="{{ $card['link'] }}" class="no-underline group">
                <div
                    class="bg-white rounded-lg p-5 border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-200 cursor-pointer">
                    <div class="flex items-center gap-4">
                        {{-- Icon Circle --}}
                        <div class="w-12 h-12 rounded-full {{ $card['bg'] }} flex items-center justify-center flex-shrink-0">
                            <i data-feather="{{ $card['icon'] }}" class="{{ $card['color'] }}"
                                style="width:22px;height:22px;stroke-width:2;"></i>
                        </div>
                        {{-- Text --}}
                        <div>
                            <p class="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-1">TOTAL
                                {{ $card['title'] }}</p>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        @foreach($actions as $action)
            <a href="{{ $action['link'] }}" class="no-underline group">
                <div
                    class="bg-white border-2 border-dashed border-gray-200 rounded-lg p-6 text-center hover:border-blue-400 hover:bg-blue-50 transition-colors duration-200 cursor-pointer">
                    <div
                        class="w-8 h-8 mx-auto mb-3 rounded-full bg-gray-100 border border-gray-200 flex items-center justify-center group-hover:bg-blue-100 group-hover:border-blue-300 transition-colors">
                        <i data-feather="plus" class="text-gray-400 group-hover:text-blue-500"
                            style="width:16px;height:16px;stroke-width:2.5;"></i>
                    </div>
