    ...
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-50">
            ...
            <tr class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-6 py-3 text-gray-600">{{ $i->phn }}</td>
                <td class="px-6 py-3 text-gray-600">{{ $i->date }}</td>
                <td class="px-6 py-3 text-gray-600">{{ $i->time }}</td>
                <td class="px-6 py-3 text-gray-500 max-w-xs truncate">{{ $i->msg }}</td>
                <td class="px-6 py-3">
                    <a href="/admin/deleteappointment/{{ $i->id }}" class="inline-flex items-center gap-1 text-xs font-medium text-red-600 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-md no-underline transition-colors"
